...
Room membership becomes part of the realtime contract, so Socket.IO handler tests should verify that events are scoped to teacher, student, and pairing audiences. Reconnect tests should verify last-socket disconnect behavior, room rejoin, and recovery of missed snapshot/chat state.
